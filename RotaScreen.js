...
import {
  ...
  ActivityIndicator, Alert, Dimensions, ScrollView, AppState
} from 'react-native';
...
const GPS_TASK = 'background-location-task';
...
async function sincronizarFila(headers) {
  try {
    ...
    if (fila.length !== restantes.length) {
      console.log(`Sincronizados ${fila.length - restantes.length} itens offline`);
    }
  } catch {}
}
...
export default function RotaScreen({ navigation, route }) {
  ...
  const [ultimoGps, setUltimoGps]   = useState(null);
  ...
  useEffect(() => {
    const unsub = NetInfo.addEventListener(state => {
      ...
      setOnline(!!isOnline);
      if (isOnline) {
        sincronizarFila(headers).then(atualizarContadorFila);
      }
    });
    return () => unsub();
  }, []);
        ...
        subscription = await Location.watchPositionAsync(
          {
            ...
            timeInterval: 15000,   // a cada 15 segundos
            distanceInterval: 50,  // ou a cada 50 metros
          },
          (loc) => {
            const novaPos = { latitude: loc.coords.latitude, longitude: loc.coords.longitude };
            setPosicao(novaPos);
            setUltimoGps(new Date());
            enviarGPS(novaPos, loc.coords.speed, loc.coords.heading);
          }
        );
      } catch {}
    }
  ...
  async function enviarGPS(coords, speed, heading) {
    if (!rotaInfo) return;
    const payload = {
      url: `${API_URL}/routes/${rotaInfo.route_id}/gps`,
      method: 'POST',
      body: {
        lat: coords.latitude,
  ...
  useEffect(() => { carregarRota(); atualizarContadorFila(); }, []);
  ...
  async function carregarRota() {
    ...
    try {
      ...
      // Tentar cache offline se não encontrou online
      if (!rotaEncontrada) {
        const cached = await AsyncStorage.getItem('rota_cache');
        if (cached) {
          const { rota, stops: stopsCache } = JSON.parse(cached);
          setRotaInfo(rota);
          setStops(stopsCache);
          setLoading(false);
          return;
      ...
      if (Array.isArray(stopsData)) {
        ...
        await AsyncStorage.setItem('rota_cache', JSON.stringify({
          rota: rotaEncontrada,
          stops: sorted,
          ts: Date.now()
        }));
        ...
        try {
          ...
          const dest_via = coords[coords.length-1];
          const origin = `${DEPOSITO.latitude},${DEPOSITO.longitude}`;
          const GKEY = 'AIzaSyB47DpEZW4qbU74LxcG1ZD76cYLRlJw88M';
          const url = `https://maps.googleapis.com/maps/api/directions/json?origin=${origin}&destination=${dest_via.latitude},${dest_via.longitude}&waypoints=${waypoints}&key=${GKEY}`;
  ...
  async function confirmarKmEIniciar() {
    ...
    try {
      const res = await fetch(`${API_URL}/routes/${rotaInfo.route_id}/iniciar`, {
        method: 'POST', headers,
        body: JSON.stringify({ km_inicial: parseInt(kmInicial) })
      });
      if (!res.ok) {
        await salvarFilaOffline({
          url: `${API_URL}/routes/${rotaInfo.route_id}/iniciar`,
          method: 'POST',
          body: { km_inicial: parseInt(kmInicial) }
        });
      }
      setRotaInfo(p => ({ ...p, status: 'executing' }));
      setShowKm(false);
    } catch (e) {
      // Offline: salvar e continuar
      await salvarFilaOffline({
        url: `${API_URL}/routes/${rotaInfo.route_id}/iniciar`,
        method: 'POST',
        body: { km_inicial: parseInt(kmInicial) }
      ...
      setRotaInfo(p => ({ ...p, status: 'executing' }));
      setShowKm(false);
      Alert.alert('Modo Offline', 'KM salvo localmente. Será sincronizado quando voltar a internet.');
    }
  }
  ...
  async function fazerLogout() {
    ...
    await AsyncStorage.removeItem('rota_cache');
      ...
      {(!online || filaCount > 0) && (
        <View style={[s.offlineBanner, { backgroundColor: online ? '#FF8C00' : '#FF3355' }]}>
          <Text style={s.offlineTxt}>
            {online
              ? `🔄 Sincronizando ${filaCount} itens...`
              : `📵 Sem internet • ${filaCount} itens na fila`
            }
          </Text>
        </View>
      )}
