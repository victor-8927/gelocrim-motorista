...
export default function RotaScreen({ navigation, route }) {
  ...
  async function carregarRota() {
    ...
    try {
      ...
      if (!rotaEncontrada) {
        ...
        assistente.mostrar('neutral', 'Sua rota ainda não foi liberada. Aguarde o analista.');
        return;
      }
      ...
      if (Array.isArray(stopsData)) {
        ...
        try {
          ...
          if (json.routes?.[0]) {
            ...
            setRouteCoords(pts ? decodePolyline(pts) : [DEPOSITO,...coords,DEPOSITO]);
          } else setRouteCoords([DEPOSITO,...coords,DEPOSITO]);
        } catch { setRouteCoords([DEPOSITO,...coords,DEPOSITO]); }
        setTimeout(() => {
          if (mapRef.current && coords.length) mapRef.current.fitToCoordinates([DEPOSITO,...coords], { edgePadding:{top:120,right:50,bottom:200,left:50}, animated:true });
        }, 800);
        const nome = user?.name?.split(' ')[0] || 'Motorista';
  ...
  async function confirmarKmEIniciar() {
    if (!kmInicial || kmInicial.length < 4) { Alert.alert('KM inválido','Informe pelo menos 4 dígitos.'); return; }
    try {
      const res = await fetch(`${API_URL}/routes/${rotaInfo.route_id}/iniciar`, { method:'POST', headers, body:JSON.stringify({km_inicial:parseInt(kmInicial)}) });
      if (!res.ok) await salvarFilaOffline({url:`${API_URL}/routes/${rotaInfo.route_id}/iniciar`,method:'POST',body:{km_inicial:parseInt(kmInicial)}});
  ...
  function getCorStop(stop) {
    ...
    if (prox?.stop_id===stop.stop_id) return NEON.cyan;
  ...
  if (showKm) return (
    <View style={[s.center,{justifyContent:'flex-start',paddingTop:50,backgroundColor:NEON.bg}]}>
      <View style={s.kmIconBox}><Text style={{fontSize:42}}>🚛</Text></View>
      ...
      {aba==='mapa' && (
        <MapView ref={mapRef} style={StyleSheet.absoluteFillObject} provider={PROVIDER_GOOGLE}
          ...
          <Marker coordinate={DEPOSITO} anchor={{x:0.5,y:0.5}}>
            <View style={s.depositoPin}><Text style={{fontSize:20}}>🏭</Text></View>
          </Marker>
          {stops.map(stop => {
            ...
            return (
              <Marker key={stop.stop_id} coordinate={{latitude:lat,longitude:lng}} onPress={()=>selecionarStop(stop)} anchor={{x:0.5,y:0.5}}>
      ...
      {aba==='lista' && (
        <View style={{flex:1,backgroundColor:NEON.bg,paddingTop:100}}>
          <ScrollView contentContainerStyle={{padding:16}}>
            {stops.map(stop=>(
              <TouchableOpacity key={stop.stop_id} style={[s.listaItem,{borderLeftColor:getCorStop(stop)}]} onPress={()=>{selecionarStop(stop);setAba('mapa');}}>
        ...
        <View style={[s.cardStop,{borderColor:getCorStop(stopSel)}]}>
          <View style={{flexDirection:'row',alignItems:'center',gap:12,marginBottom:10}}>
            ...
            <View style={{flex:1}}>
              ...
              <Text style={s.cardPeso}>⚖️ {(stopSel.weight_kg||0).toFixed(0)} kg</Text>
            </View>
          ...
          <TouchableOpacity
            ...
            onPress={()=>navigation.navigate('Entrega',{
              ...
              onStopUpdated:(stopId,status)=>{
                setStops(prev=>prev.map(s=>s.stop_id===stopId?{...s,status}:s));
                setStopSel(null);
                if(status==='completed') assistente.anunciarSucesso(stopSel.recipient_name);
                else assistente.anunciarCrise(status==='failed'?'Não entregue':'Reentrega solicitada');
              }
            })}
          >
            <Text style={s.btnEntrarTxt}>{rotaInfo?.status==='executing'?'📦 REGISTRAR ENTREGA →':'Inicie a operação primeiro'}</Text>
          </TouchableOpacity>
        </View>
      )}

      <View style={s.bottomBar}>
        {[{id:'mapa',icon:'🗺️',label:'Mapa'},{id:'lista',icon:'📋',label:'Lista'}].map(item=>(
          <TouchableOpacity key={item.id} style={[s.tabBtn,aba===item.id&&s.tabBtnAtivo]} onPress={()=>{setAba(item.id);setStopSel(null);}}>
            <Text style={{fontSize:22}}>{item.icon}</Text>
            <Text style={[s.tabLabel,aba===item.id&&{color:NEON.cyan}]}>{item.label}</Text>
          </TouchableOpacity>
        ))}
        {rotaInfo?.status==='executing'&&(
          <TouchableOpacity style={s.tabBtnFinalizar} onPress={()=>Alert.alert('Finalizar Viagem','Confirma que chegou à fábrica?',[{text:'Cancelar',style:'cancel'},{text:'Sim, chegei!',onPress:()=>navigation.navigate('Resumo',{token,routeId:rotaInfo.route_id,stops,kmInicial})}])}>
